...
func Decode(carrier io.Reader, result io.Writer) error {
	...
	for x := 0; x < dx && dataCount > 0; x++ {
		for y := 0; y < dy && dataCount > 0; y++ {
			if count >= dataSizeHeaderReservedBytes {
				...
				dataBytes = append(dataBytes, utils.GetLastTwoBits(c.R), utils.GetLastTwoBits(c.G), utils.GetLastTwoBits(c.B))
				dataCount -= 3
			} else {
				count += 4
			}
	...
	for i := 0; i < len(dataBytes); i += 4 {
		resultBytes = append(resultBytes, utils.ConstructByteOfQuartersAsSlice(dataBytes[i:i+4]))
	}
...
func extractDataCount(RGBAImage *image.RGBA) int {
	...
	for x := 0; x < dx && count < dataSizeHeaderReservedBytes; x++ {
		for y := 0; y < dy && count < dataSizeHeaderReservedBytes; y++ {
			c := RGBAImage.RGBAAt(x, y)
			dataCountBytes = append(dataCountBytes, utils.GetLastTwoBits(c.R), utils.GetLastTwoBits(c.G), utils.GetLastTwoBits(c.B))
			count += 4
		}
	}
	...
	var bs = []byte{utils.ConstructByteOfQuartersAsSlice(dataCountBytes[:4]),
		utils.ConstructByteOfQuartersAsSlice(dataCountBytes[4:8]),
		utils.ConstructByteOfQuartersAsSlice(dataCountBytes[8:12]),
		utils.ConstructByteOfQuartersAsSlice(dataCountBytes[12:])}
