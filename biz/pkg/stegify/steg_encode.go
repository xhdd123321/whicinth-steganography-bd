...
func quartersOfBytesOf(counter uint32) []byte {
	...
	for i := 0; i < 16; i += 4 {
		quarters[i] = utils.QuartersOfByte(bs[i/4])[0]
		quarters[i+1] = utils.QuartersOfByte(bs[i/4])[1]
		quarters[i+2] = utils.QuartersOfByte(bs[i/4])[2]
		quarters[i+3] = utils.QuartersOfByte(bs[i/4])[3]
	}
...
func setDataSizeHeader(RGBAImage *image.RGBA, dataCountBytes []byte) {
	...
	for x := 0; x < dx && count < (dataSizeHeaderReservedBytes/4)*3; x++ {
		for y := 0; y < dy && count < (dataSizeHeaderReservedBytes/4)*3; y++ {
			c := RGBAImage.RGBAAt(x, y)
			c.R = utils.SetLastTwoBits(c.R, dataCountBytes[count])
			c.G = utils.SetLastTwoBits(c.G, dataCountBytes[count+1])
			c.B = utils.SetLastTwoBits(c.B, dataCountBytes[count+2])
			RGBAImage.SetRGBA(x, y, c)
...
func setColorSegment(colorSegment *byte, data <-chan byte, errChan <-chan error) (hasMoreBytes bool, err error) {
	...
	case byte, ok := <-data:
		...
		*colorSegment = utils.SetLastTwoBits(*colorSegment, byte)
		return true, nil
...
func readData(reader io.Reader, bytes chan<- byte, errChan chan<- error) {
	...
	for {
		...
		for _, b := range utils.QuartersOfByte(b[0]) {
			bytes <- b
		}
